...
class CockpitAgent {
  ...
  analyzeIntent(message) {
    ...
    // Parse message with compromise.js for NLP analysis
    ...
    // Detect intent from verbs using NLP
    const verbIntent = this.detectIntentFromVerbs(nlpData);
    
    // Extract quantity using NLP
    const quantity = this.extractQuantity(nlpData, text);

    // Recent/Show orders intent - CHECK THIS FIRST before product search
    if (
      text.includes("recent order") ||
      text.includes("show order") ||
      text.includes("my order") ||
      text.includes("order history") ||
      text.includes("past order") ||
      (text.includes("show") && nlpData.nouns.some(n => n.toLowerCase().includes('order'))) ||
      (verbIntent === 'show_orders' && nlpData.nouns.some(n => n.toLowerCase().includes('order')))
    ) {
      return { type: "show_orders", entities: {} };
    }

    // Product search intent - more general, check after specific intents
    if (
      verbIntent === 'search' ||
      text.includes("search") ||
      text.includes("find") ||
      (text.includes("look for") && !text.includes("order"))
    ) {
      const searchEntities = this.extractSearchQuery(message);
      // Add corrected slots to search entities
      searchEntities.slots = slots;
      return { type: "product_search", entities: searchEntities };
    }
    
    // Wishlist intent
    if (
      verbIntent === 'add_to_wishlist' ||
      text.includes("add to wishlist") ||
      text.includes("wishlist") ||
      text.includes("save for later") ||
      nlpData.nouns.some(n => n.toLowerCase().includes('wishlist'))
    ) {
      const entities = this.extractCartEntities(message, nlpData, quantity);
    ...
    // Add to cart intent
    if (
      verbIntent === 'add_to_cart' ||
      text.includes("add to cart") ||
      text.includes("add product") ||
      text.includes("add item") ||
      (nlpData.verbs.some(v => v.toLowerCase() === 'add') && 
       nlpData.nouns.some(n => ['cart', 'product', 'item'].includes(n.toLowerCase())))
    ) {
      const entities = this.extractCartEntities(message, nlpData, quantity);
      entities.slots = slots;
      return { 
    ...
    // Orders intent (general) - less specific than show_orders
    if (
      verbIntent === 'order' ||
      text.includes("order") || 
    ...
    if (
      ...
      nlpData.nouns.some(n => n.toLowerCase().includes('return'))
    ) {
    ...
    if (
      ...
      nlpData.nouns.some(n => n.toLowerCase().includes('payment'))
    ) {
